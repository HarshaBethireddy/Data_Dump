...
class AppIDConfig(BaseModel):
    ...
    @field_validator('end_range')
    @classmethod
    def validate_ranges(cls, v, info):
        """Validate that end_range is greater than start_range."""
        if info.data and 'start_range' in info.data and v <= info.data['start_range']:
            raise ValueError('end_range must be greater than start_range')
        return v
    
    @field_validator('prequal_end_range')
    @classmethod
    def validate_prequal_ranges(cls, v, info):
        """Validate that prequal_end_range is greater than prequal_start_range."""
        if info.data and 'prequal_start_range' in info.data and v <= info.data['prequal_start_range']:
